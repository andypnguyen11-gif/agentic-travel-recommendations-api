...
- [x] **1. Skeleton + health** — FastAPI app, `/health`, `config.py`
  (pydantic-settings), `requirements.txt`, `.gitignore`, `.env.example`,
  `pytest.ini`. Test: `/health` returns ok; app boots.

- [x] **2. Domain models + errors** — enums (LoyaltyTier, Category, Source),
...
- [x] **8. Tenant guard + orchestration service** — `authorize(ctx, member)`;
...
- [x] **9. Structured logging (PII-safe)** — `configure_logging` + `safe_log_fields`
  (allowlist only). Tests: allowlist keys only; no PII keys leak.

- [x] **10. REST endpoint** — `POST /recommendations` with header-built context;
...
- [x] **11. MCP stdio server** — FastMCP, two tools (`member_id` only), env-based
...
- [x] **12. Eval suite** — `tests/evals/`: tenant isolation (both tools), cruise
  exclusion (Platinum), cap enforcement, fail-closed. Run via `pytest tests/evals`.

- [x] **13. CLI demo** — `cli/demo.py` end-to-end flow; prints tier/partner/recs/
  applied-rules/removed. Tests: happy path, cross-partner safe error, unknown member.

- [x] **14. Docker + compose** — `Dockerfile`, `docker-compose.yml`; `/health` and
  `/recommendations` work in the container.

- [x] **15. README (A/B/C)** — overview, setup, REST examples, **MCP Inspector demo
...
- [x] **16. Final verification sweep** — full `pytest`, evals alone, exercise all
